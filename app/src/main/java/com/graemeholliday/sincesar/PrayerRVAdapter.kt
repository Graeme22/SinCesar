...
class PrayerRVAdapter(
    val noteClickDeleteInterface: NoteClickDeleteInterface,
    val noteClickInterface: NoteClickInterface
) :
    RecyclerView.Adapter<PrayerRVAdapter.ViewHolder>() {

    //on below line we are creating a variable for our all notes list.
    private val allNotes = ArrayList<Prayer>()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        var desc = allNotes.get(position).description.split("\n")[0]
        holder.dateTV.setText(if(desc.length > MAX_LENGTH) desc.substring(0..MAX_LENGTH) + "..." else desc)
    ...
    fun updateList(newList: List<Prayer>) {
